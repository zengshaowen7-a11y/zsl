...
export default function ContactPage() {
  return <main className="ctw-page" id="top">
    ...
    <section className="ctw-prepare"><div className="ctw-container"><div><p className="ctw-kicker">WHAT HELPS US REVIEW FASTER</p><h2>Four details help us reply faster.</h2></div><ol>{["Product or supplier link","Main destination markets","Expected daily order range","Packaging or QC requirements"].map((item,index)=><li key={item}><span>{String(index+1).padStart(2,"0")}</span>{item}</li>)}</ol></div></section>
