...
export function FulfillmentAbout({ lang = "en" }) {
  ...
  return (
    <main className="ff-site">
      <InnerHero copy={c} lang={lang} />
      <section className="ff-section ff-about-story"><div className="container ff-about-story-grid"><div><span className="ff-kicker">{c.storyEyebrow}</span><h2>{c.storyTitle}</h2><p>{c.storyLead}</p></div><div className="ff-about-image"><Image src="/images/jw-content/delivery-fleet.webp" alt="Global shipping containers" fill sizes="(max-width: 1024px) 100vw, 50vw" /><span><FiShield />{isZh ? "从中国连接全球市场" : "Connecting China to global markets"}</span></div></div></section>
