...
const serviceTitleClass = (title) => title.length > 48 ? "sdr-title-long" : undefined;
...
export default function ServiceDetailRedesign({ service }) {
  ...
  return (
    <main className={`ff-site sdr-page sdr-page-${service.slug}`}>
      ...
      <section className="sdr-scope"><div className="container"><div className="sdr-heading-split"><div><span className="ff-kicker">WHAT WE HANDLE</span><h2 className={serviceTitleClass(service.capabilitiesTitle)}>{service.capabilitiesTitle}</h2></div><p>{service.capabilitiesLead}</p></div><div className="sdr-scope-grid">{service.capabilities.map(([title, text], index) => { const Icon = capabilityIcons[index]; return <article key={title}><span><Icon /></span><div><h3>{title}</h3><p>{text}</p></div></article>; })}</div></div></section>

      <section className="sdr-proof"><div className="container sdr-proof-grid"><figure><Image src={proof.image} alt={proof.title} fill sizes="(max-width: 900px) 100vw, 45vw" unoptimized={proof.image.includes("/generated/")} /></figure><div className="sdr-proof-content"><span className="ff-kicker">{proof.eyebrow}</span><h2 className={serviceTitleClass(proof.title)}>{proof.title}</h2><div className="sdr-proof-table"><div className="sdr-proof-row sdr-proof-head">{proof.columns.map((column) => <strong key={column}>{column}</strong>)}</div>{proof.rows.map((row, index) => <div className="sdr-proof-row" key={index}>{row.map((cell, cellIndex) => <span key={`${cell}-${cellIndex}`}>{cell}</span>)}</div>)}</div><small>{proof.note}</small></div></div></section>

      <section id="service-process" className="sdr-process"><div className="container"><div className="sdr-section-heading"><span className="ff-kicker">{service.processEyebrow}</span><h2 className={serviceTitleClass(service.processTitle)}>{service.processTitle}</h2><p>{service.processLead}</p></div><div className="sdr-process-grid">{service.process.map(([number, title, text]) => <article key={number}><span>{number}</span><h3>{title}</h3><p>{text}</p></article>)}</div></div></section>

      <section className="sdr-planning"><div className="container"><div className="sdr-heading-split"><div><span className="ff-kicker">{service.spotlight.eyebrow}</span><h2 className={serviceTitleClass(service.spotlight.title)}>{service.spotlight.title}</h2></div><p>{service.spotlight.lead}</p></div><div className="sdr-planning-grid">{service.spotlight.items.map(([title, text], index) => <article key={title}><small>0{index + 1}</small><h3>{title}</h3><p>{text}</p></article>)}</div></div></section>

      <section className="sdr-case"><div className="container sdr-case-grid"><div><span className="ff-kicker">{conversion.caseStudy.eyebrow}</span><h2 className={serviceTitleClass(conversion.caseStudy.title)}>{conversion.caseStudy.title}</h2><p><FiGlobe />{conversion.caseStudy.profile}</p></div><div className="sdr-case-board"><strong>THE OPERATING CHALLENGE</strong><p>{conversion.caseStudy.challenge}</p><ol>{conversion.caseStudy.plan.map((step, index) => <li key={step}><span>0{index + 1}</span>{step}</li>)}</ol><blockquote>{conversion.caseStudy.outcome}</blockquote></div></div></section>

      <section className="sdr-faq"><div className="container sdr-faq-grid"><div><span className="ff-kicker">SERVICE FAQ</span><h2 className={serviceTitleClass(`Questions about ${service.menuTitle}?`)}>Questions about {service.menuTitle}?</h2><p>The exact scope depends on your product, markets, volume and customer promise.</p></div><div>{service.faqs.slice(0, 5).map(([question, answer], index) => <details name={`${service.slug}-faq`} key={question} open={index === 0}><summary>{question}<span>+</span></summary><p>{answer}</p></details>)}</div></div></section>
      ...
      <section className="sdr-quote"><div className="container sdr-quote-grid"><div><span className="ff-kicker">{conversion.form.eyebrow}</span><h2 className={serviceTitleClass(conversion.form.title)}>{conversion.form.title}</h2><p>{conversion.form.lead}</p></div><ContactForm id="service-quote" className="sdr-form" source={`service-page:${service.slug}`} service={service.menuTitle} /></div></section>
    </main>
  );
}
